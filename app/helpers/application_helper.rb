...
module ApplicationHelper
  ...
  def verbose_replies number, type='default'
    if type == 'new'
      ...
      result = number.to_s
      result = 'непрочитанны' if type == 'unread'
      number_mod = number % 10
      if (2..4).include? number_mod and not (12..14).include? number
        return result += 'x' if type == 'unread'
        result += ' ответа'
      elsif number_mod != 1 or number == 11
        return result += 'x' if type == 'unread'
        result += ' ответов'    
      else
        return result += 'й' if type == 'unread'
        result += ' ответ'
      end
      result = 'нет ответов' if number == 0
      return result      
    end
  end
  ...
  def tree?
    if @settings['settings'].has_key? 'layout'
